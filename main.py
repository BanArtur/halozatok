import argparse
from algorithm import prepare_decompose, run_list_model, spiderNonAdaptive, transform_to_list, treeDecompose
    ...
    def drawGraph(index,pos):
        ...
        for u,v in G.edges():
            if list_model[index][0].edges[(u,v)].secret_cost is not None:
                edgeLabelMap[(u,v)] = f"{list_model[index][0].edges[(u,v)].secret_cost:.2f}"
            else:
                edgeLabelMap[(u,v)] = str(list_model[index][0].edges[(u,v)].cost_distribution)
        ax.clear()
...
def main():
    ...
    history = solve_spider_graphs(graph) if args.mode == "spider" else solve_bounded_trees(graph)
    show(history)
